<template>
  <div class="product-card rounded-xl overflow-hidden unsc-hover group">
    <div class="relative">
      <img :src="product.image" :alt="product.name" class="w-full h-52 object-contain bg-black/20">
      <div v-if="product.discount" class="absolute top-2 left-2 bg-red-600 text-white text-xs px-2 py-1 rounded">
        -{{ product.discount }}%
      </div>
      <div v-if="product.newBadge" class="absolute top-2 left-2 bg-green-600 text-white text-xs px-2 py-1 rounded">
        {{ product.newBadge }}
      </div>
      <div v-if="product.specialBadge" class="absolute top-2 left-2 text-white text-xs px-2 py-1 rounded" :class="product.specialBadge.class">
    ...
    <div class="p-4">
      ...
      <h3 class="font-bold text-lg mb-1 group-hover:text-primary transition">{{ product.name }}</h3>
      ...
      <div class="flex items-center justify-between">
        <div v-if="product.originalPrice">
          ...
          <p class="text-xl font-bold text-primary">${{ product.price }} MXN</p>
        </div>
        <div v-else>
          <p class="text-xl font-bold text-primary">${{ product.price }} MXN</p>
        </div>
